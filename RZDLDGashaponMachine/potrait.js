...
var timeouts;
	...
	function createImage(){
		...
		// 做红色鱼的function
		function redFishImg(id) {
			for (i = 0; i < redFishArray.length; i++) {
				if (i < redFishArray.length) {
					var Image = document.createElement("IMG");
					Image.setAttribute("src", redFishArray[i]);
					Image.setAttribute("class", id.id);
					id.appendChild(Image);
				}
			}
		}
		// 做蓝色鱼的function
		function blueFishImg(id) {
			for (i = 0; i < blueFishArray.length; i++) {
				if (i < blueFishArray.length) {
					var Image = document.createElement("IMG");
					Image.setAttribute("src", blueFishArray[i]);
					Image.setAttribute("class", id.id);
					id.appendChild(Image);
				}
			}
		...
		// 做黑色鱼的function
		function blackFishImg(id) {
			for (i = 0; i < blackFishArray.length; i++) {
				if (i < blackFishArray.length) {
					var Image = document.createElement("IMG");
					Image.setAttribute("src", blackFishArray[i]);
					Image.setAttribute("class", id.id);
					id.appendChild(Image);
				}
			}
		...
		redFishImg(fishRed1);
		redFishImg(fishRed2);
		blackFishImg(fishBlack1);
		blackFishImg(fishBlack2);
		blueFishImg(fishBlue1);
		blueFishImg(fishBlue2);
		...
		// 鱼的Animation
		function fishAnimation(id) {
			var myIndex = 0;
			var img = document.getElementsByClassName(id.id);
			var render;
			loopEffect();
			function loopEffect() {
		...
		// call Animation
		fishAnimation(fishRed1);
		fishAnimation(fishRed2);
		fishAnimation(fishBlack1);
		fishAnimation(fishBlack2);
		fishAnimation(fishBlue1);
		fishAnimation(fishBlue2);
		...
		startButton.addEventListener("click", function(){
			...
			createFish(fishRed1);
			createFish(fishRed2);
			createFish(fishBlack1);
			createFish(fishBlack2);
			createBlueFish(fishBlue1);
			createBlueFish(fishBlue2);
			starEffectAnimation();
		});
	}		
	...
	function createFish(id) {
		...
		var y = Math.random();
		var z = Math.random();
		fish = document.querySelector("#"+id.id);
		if (x < 0.5) {
			fish.style.transform = "scaleX(-1)";
			fishmoveRight(id, Math.random()+0.2);
		}else {
			fish.style.transform = "scaleX(1)";
			fishmoveLeft(id, Math.random()+0.2);	
		}
		if (y < 0.3) {
			fish.style.top = "57%";
		}else if (y >= 0.3 && y <= 0.5) {
		...
		if (z < 0.5) {
			fish.style.height = "16%";
			fish.style.width = "28%";
		}else {
			fish.style.height = "32%";
			fish.style.width = "55%";
		}
	}
	
	function createBlueFish(id) {
		var x = Math.random();
		var y = Math.random();
		var z = Math.random();
		fishblue = document.querySelector("#"+id.id);
		if (x < 0.5) {
			fishblue.style.transform = "scaleX(1)";
			fishmoveRightBlue(id, Math.random()+0.2);
		}else {
			fishblue.style.transform = "scaleX(-1)";
			fishmoveLeftBlue(id, Math.random()+0.2);	
		}
		if (y < 0.3) {
			fishblue.style.top = "60%";
		}else if (y >= 0.3 && y <= 0.5) {
			fishblue.style.top = "30%";
		}else if (y > 0.5 && y <= 0.75) {
			fishblue.style.top = "85%";
		}else if (y > 0.75 && y < 1) {
			fishblue.style.top = "70%";
		}
		if (z < 0.5) {
			fishblue.style.height = "16%";
			fishblue.style.width = "28%";
		}else {
			fishblue.style.height = "32%";
			fishblue.style.width = "55%";
		}
	}
	
	
	function fishmoveLeft(id, speed) {
		var pos = 100;
		moveLeft(id, speed);
		function moveLeft(id, speed){
			fishLeft = document.querySelector("#"+id.id);
			pos-=speed;
			fishLeft.style.left = pos+"%";
			move = requestAnimationFrame(function (timestamp) {
				moveLeft(id, speed);
			});
			if (pos < -50) {
	...
	function fishmoveRight(id, speed) {
		var pos = -50;
		moveRight(id, speed);
		function moveRight(id, speed){
			fishRight = document.querySelector("#"+id.id);
			pos+=speed;
			fishRight.style.left = pos+"%";
			move = requestAnimationFrame(function (timestamp) {
				moveRight(id, speed);
			});
			if (pos > 100) {
	...
	function fishmoveLeftBlue(id, speed) {
		var pos = 100;
		moveLeft(id, speed);
		function moveLeft(id, speed){
			fishLeft = document.querySelector("#"+id.id);
	...
	function fishmoveRightBlue(id, speed) {
		var pos = -50;
		moveRight(id, speed);
		function moveRight(id, speed){
			fishRight = document.querySelector("#"+id.id);
